...
def _sheet_dashboard(wb):
    ...
    phases = [
        ["Phase 1 – Local Setup",       "Days 1–7",   "Bot running in paper mode",                         "Not Started", ""],
        ["Phase 2 – Fix Critical Issues","Days 8–37",  "Security fixed, bugs corrected, Grafana live",      "Not Started", ""],
        ["Phase 3 – New Strategies",    "Days 38–82", "8 new strategies built and backtested",             "Not Started", ""],
        ["Phase 4 – Paper Validation",  "Days 83–97", "Sharpe > 0.8, Drawdown < 8%, Win rate > 45%",      "Not Started", ""],
        ["Phase 5 – Cloud Migration",   "Days 98+",   "Hetzner server, systemd, live trading with $1,000", "Not Started", ""],
    ]
...
def _phase1_tasks():
    return [
        "INSTALLATION",
        ("Verify Python 3.12.x installed",          "PowerShell",                 "Day 1",   "30 min", "CRITICAL", "Not Started", "Run: python --version"),
        ("Install Poetry package manager",           "PowerShell",                 "Day 1",   "30 min", "CRITICAL", "Not Started", "Run: pip install poetry"),
        ("Run poetry install",                       "Project root",               "Day 1",   "1 hr",   "CRITICAL", "Not Started", "Wait for all libraries to download"),
        "SECURITY — DO THIS BEFORE ANYTHING ELSE",
        ("Rotate leaked API keys in .env",           ".env",                       "Day 2",   "1 hr",   "CRITICAL", "Not Started", "DELETE all content, paste safe template"),
        ("Register on Delta testnet",                "cdn-ind.testnet.deltaex.org","Day 2",   "30 min", "CRITICAL", "Not Started", "Get fresh key + secret"),
        ("Update .env with testnet credentials",     ".env",                       "Day 2",   "10 min", "CRITICAL", "Not Started", "DELTA_EXCHANGE_ENV=testnet-india"),
        "FIRST RUN",
        ("Run bot in paper mode",                    "cli/main.py",                "Day 2",   "30 min", "HIGH",     "Not Started", "poetry run python -m delta_exchange_bot.cli.main --mode paper"),
        ("Observe log output for 30 min",            "Terminal",                   "Day 2",   "30 min", "HIGH",     "Not Started", "Look for JSON lines every 60s"),
        ("Verify DB has trade/signal rows",          "state.db",                   "Day 2",   "15 min", "HIGH",     "Not Started", "Use SQLite Viewer VS Code extension"),
        "OBSERVATION (Days 3–7)",
        ("Run paper mode 2+ hours daily",            "Terminal",                   "Days 3–7","Passive","MED",      "Not Started", "Note any error messages"),
        ("Confirm bot runs 2 consecutive days clean","Terminal",                   "Day 7",   "–",      "HIGH",     "Not Started", "Phase 1 complete gate"),
    ]
...
def _phase2_tasks():
    return [
        "CRITICAL SEVERITY",
        ("Fix Dockerfile Python 3.11 → 3.12",        "Dockerfile line 1",                         "Day 8",    "30 min", "CRITICAL", "Not Started", "python:3.11-slim → python:3.12-slim"),
        ("Merge strategy/ and strategies/ packages",  "src/delta_exchange_bot/strategy/",          "Days 8–10","3 days", "CRITICAL", "Not Started", "Two RSIScalping classes cause shadow bugs"),
        ("Update all imports after merge",            "core/engine.py, portfolio.py",              "Day 10",   "1 day",  "CRITICAL", "Not Started", "Run pytest after — all must pass"),
        ("Change TRADE_FREQUENCY_S 60 → 5",          ".env, core/settings.py",                    "Day 8",    "30 min", "CRITICAL", "Not Started", "Add signal_eval_every=6 setting"),
        ("Add exchange-native stop orders",           "api/delta_client.py",                       "Days 14–20","6 days","CRITICAL", "Not Started", "place_stop_limit_order() method"),
        ("Store stop_order_id in DB",                 "persistence/models.py",                     "Day 15",   "1 day",  "CRITICAL", "Not Started", "Add column to Position model"),
        ("Cancel stop order on position close",       "execution/order_execution_engine.py",       "Day 16",   "1 day",  "CRITICAL", "Not Started", "Prevent orphaned stop orders"),
        ...
        ("Wire YAML config files to Settings",        "core/settings.py",                          "Days 10–12","2 days","HIGH",     "Not Started", "config/paper.yml currently ignored"),
        ("Consolidate two risk managers into one",    "risk/risk_manager.py",                      "Day 11",   "1 day",  "HIGH",     "Not Started", "Delete advanced_risk_manager.py"),
        ("Add fill confirmation polling loop",        "execution/order_execution_engine.py",       "Day 13",   "1 day",  "HIGH",     "Not Started", "Poll until filled/rejected/timeout"),
        "MEDIUM SEVERITY",
        ...
        ("Add max holding period (30 min default)",   "core/engine.py, core/settings.py",          "Days 24–26","2 days","MED",      "Not Started", "max_hold_candles=30 setting"),
        ("Persist ProtectionState to PostgreSQL",     "persistence/models.py, db.py",              "Day 20",   "2 days", "MED",      "Not Started", "Crash-safe SL/TP recovery"),
        ("Add crash recovery on startup",             "core/engine.py",                            "Day 21",   "1 day",  "MED",      "Not Started", "_recover_state() method"),
        "MONITORING",
...
def _sheet_known_issues(wb):
    ...
    issues = [
        ["CRIT", "Real API keys committed to GitHub",       ".env",                                  "Phase 1 Day 2", "Not Fixed", "Security — exchange account at risk",   "Delete .env content immediately"],
        ["CRIT", "SL/TP only in memory — lost on crash",   "execution/order_execution_engine.py",   "Phase 2",       "Not Fixed", "Unprotected positions after restart",   "Place real stop orders on exchange"],
        ["HIGH", "Dockerfile uses Python 3.11 not 3.12",   "Dockerfile line 1",                     "Phase 2 Day 8", "Not Fixed", "Docker build will fail",                "One-line fix"],
        ...
        ["HIGH", "Duplicate strategy/ and strategies/",    "src/ directory",                         "Phase 2 Day 8", "Not Fixed", "Two classes with same name → bugs",     "Merge into strategies/ package"],
        ["HIGH", "TRADE_FREQUENCY_S=60 — too slow",        ".env",                                   "Phase 2 Day 8", "Not Fixed", "Scalp opportunities gone by next poll", "Change to 5s poll, 30s signal eval"],
        ["MED",  "YAML config files ignored by Settings",  "core/settings.py",                      "Phase 2 Day 10","Not Fixed", "config/paper.yml has no effect",        "Add from_yaml_and_env() loader"],
        ...
        ["MED",  "No max holding period for positions",    "core/engine.py",                         "Phase 2 Day 24","Not Fixed", "Positions can be open forever",         "30-candle default (30 min on 1m)"],
        ...
        ["MED",  "No fill confirmation polling",           "execution/order_execution_engine.py",   "Phase 2",       "Not Fixed", "Order could be rejected silently",      "Poll status until filled/timeout"],
